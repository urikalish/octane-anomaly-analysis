...
function getHistory(entityId) {
	return new Promise((resolve /*, reject*/) => {
		...
		(result) => {
			//console.log(`Retrieved ${result.data ? result.data.length : '0'} history records - ${(new Date()).getTime() - startTime}ms`);
			resolve(result);
		},
		(reason) => {
...
function getDefects(limit) {
	return new Promise((resolve /*, reject*/) => {
		...
		//console.log(`Retrieving defects...`);
		//let startTime = (new Date()).getTime();
		apiService.getEntity(uri).then(
			(result) => {
				//console.log(`Retrieved ${result.data ? result.data.length : '0'} defects (out of ${result['total_count']}) - ${(new Date()).getTime() - startTime}ms`);
				resolve(result);
			},
			(reason) => {
